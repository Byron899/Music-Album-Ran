...
export default function MusicRankingApp() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!albumResponse.ok) throw new Error("Failed to save album");
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto">
      <h1 className="text-2xl font-bold mb-4">Music Album Ranking</h1>
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <Input name="album_type" value={albumForm.album_type} onChange={handleAlbumChange} placeholder="Album Type (Album, Mixtape, EP, etc.)" required />
        <h2 className="text-lg font-bold mt-4">Songs</h2>
      ...
      <h2 className="text-xl font-bold mt-6">Ranked Albums</h2>
      {albums.map((album, index) => (
        <Card key={index} className="mt-2">
          <CardContent className="p-4">
            <p className="font-bold">{album.album} ({album.short_name || "N/A"}) - {album.artist}</p>
